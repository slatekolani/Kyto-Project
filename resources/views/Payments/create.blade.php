...
{{--                                                                        <h4 style="padding-left: 30px;"> {{\App\Models\paymentGateways\paymentGateways::find($tour_operator_account->payment_gateway)->payment_gateway_name}}</h4>--}}
                                                        ...
                                                        <div class="modal-body">
                                                            {{ Form::open([ 'route' => 'payments.store',  'autocomplete' => 'off','method' => 'post', 'class' => 'needs-validation', 'validate']) }}
                                                            ...
                                                            <input name="tour_operators_id" value="{{$tourist_booking->tourOperators->id}}" hidden>
                                                            <input name="tourist_bookings_id" value="{{$tourist_booking->id}}" hidden>
        ...
        @endforeach
    @else
        <span> Sorry, Seems {{$tourist_booking->tourOperators->company_name}} has not published their accounts public. Please mail the tour operator <a href="mailTo:{{$tourist_booking->tourOperators->email_address}}"> here </a>  or call <a href="tel:{{$tourist_booking->tourOperators->phone_number}}">here</a> to get instructions on payments</span>
    @endif
</div>
